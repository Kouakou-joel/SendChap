...
const trierBugs = (bugs: any[], option: string) => {
    if (option === 'asc') {
        return [...bugs].sort((a, b) => a.id - b.id);
    } else if (option === 'desc') {
        return [...bugs].sort((a, b) => b.id - a.id);
    }
    return bugs;
};

const recupererEtAfficherBugs = async (setBugs: any, setTotalPages: any, itemsPerPage: number) => {
    try {
        const response = await axios.get('/api/bugs');
        setBugs(response.data);
        setTotalPages(Math.ceil(response.data.length / itemsPerPage));
    } catch (error) {
        console.error("Il y a eu une erreur lors de la récupération des bugs!", error);
    }
};
...
export default function GestionBug() {
    ...
    useEffect(() => {
        recupererEtAfficherBugs(setBugs, setTotalPages, itemsPerPage);
    }, []);
            ...
            <DashboardLayout gestionbBug={'Gestion des bug'} suptitle={'Suivre l\'etat des problemes sur notre application .'}>
                ...
                <nav className="flex justify-between items-center bg-transparent px-4">
                    ...
                    <div className="flex bg-orange-600 from-white to-white p-1 bg-clip-border border rounded-xl">
                        <a href="/signal-bug" className="px-2 py-1 text-white">Signaler un bug</a>
                    </div>
                </nav>
                <div className="mt-14">
                        ...
                        <div className="overflow-x-auto">
                            <table className="bg-white border min-w-full">
                                ...
                                <tbody className='items-center'>
                                    {trierBugs(bugs, sortOption).map((bug: any) => (
                                        <tr key={bug.id} className="items-center hover:bg-gray-50 border-b text-gray-600 text-sm">
                                            <td className="border-gray-300 py-2 text-center">
                                                <input type="checkbox" className="cursor-pointer" />
                                            </td>
                                            <td className="flex px-4 py-2">
                                                <h3 className='flex items-center'>{bug.id}</h3>
                                            </td>
                                            <td className="px-4 py-2">{bug.description}</td>
                                            <td className="px-4 py-2">  </td>
                                            <td className="px-2 py-2">
                                                <span className={`inline-flex items-center px-2 py-1 rounded-md ring-inset font-medium text-white ${bug.priority === 'Critique' ? 'bg-orange-500 ring-orange-500' : bug.priority === 'Moyenne' ? 'bg-yellow-500 ring-yellow-500' : 'bg-green-200 ring-green-100'}`}>
                                                    {bug.priority}
                                                </span>
                                            </td>
                                            <td className="px-2 py-2">
                                                <span className={`inline-flex items-center px-2 py-1 rounded-md ring-inset font-medium text-white ${bug.status === 'Ouvert' ? 'bg-red-500 ring-red-500' : bug.status === 'En cours' ? 'bg-yellow-500 ring-yellow-500' : 'bg-green-200 ring-green-100'}`} >
                                                    {bug.status}
                                                </span>
                                            </td>
                            ...
                            <div className="flex justify-between items-center px-4 py-6">
                                <button
                                    className={`flex items-center gap-2 px-3 py-1 rounded-md ${
                                        currentPage === 1 ? 'bg-gray-200 text-gray-400' : 'border border-gray-300 text-gray-600'
                                    }`}
                                ...
                                <button
                                    className={`flex items-center gap-2 px-3 py-1 rounded-md ${
                                        currentPage === totalPages ? 'bg-gray-200 text-gray-400' : 'border border-gray-300 text-gray-600'
                                    }`}
                                    onClick={() => handlePageChange(currentPage + 1)}
